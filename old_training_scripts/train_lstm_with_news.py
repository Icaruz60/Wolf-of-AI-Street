...
@dataclass
class Config:
    # Path to the merged price+news CSV
    ...
    # News feature columns (all optional but expected to exist in your file)
    score_column: str = "mean_score"       # aggregated sentiment / score
    tone_column: str = "mean_tone"         # optional sentiment tone
    n_articles_column: str = "n_articles"  # article count per hour

    # Sequence / target
    sequence_length: int = 24   # past hours as input
    horizon: int = 1            # predict 1 hour ahead return
    max_return: float = 0.01    # scale factor for return normalization
...
class PriceNewsDataset(Dataset):
    """
    Input:
      sequence of features over `sequence_length` hours:
        - normalized close price
        - normalized news score (if present)
        - normalized article count
        - has_news flag (0/1)
        - normalized tone (if present)

    Target:
      normalized 1-step-ahead return, clipped to [-1, 1].
    """
    ...
    def __init__(self, df: pd.DataFrame, cfg: Config):
        ...
        close_std = close.std() + 1e-8
        close_norm = (close - close_mean) / close_std

        # ----- News features -----
        # If columns are missing, we replace them with zeros.
        def safe_col(name: str):
            if name in df.columns:
                return df[name].astype(float).fillna(0.0).values
            else:
                return np.zeros_like(close)

        score = safe_col(cfg.score_column)
        tone = safe_col(cfg.tone_column)
        n_articles = safe_col(cfg.n_articles_column)

        # has_news flag: 1 if there was at least one article that hour
        has_news = (n_articles > 0).astype(float)

        # Normalize numeric news features (except has_news)
        def norm_vec(x: np.ndarray):
            std = x.std()
            if std < 1e-8:
                return np.zeros_like(x)
            return (x - x.mean()) / (std + 1e-8)

        score_norm = norm_vec(score)
        tone_norm = norm_vec(tone)
        n_articles_norm = norm_vec(n_articles)

        # Stack features: [close, score, n_articles, has_news, tone]
        self.features = np.stack(
            [close_norm, score_norm, n_articles_norm, has_news, tone_norm],
            axis=1,
        )
    ...
    def __getitem__(self, index: int):
        ...
        y_tensor = torch.tensor(normalized_return, dtype=torch.float32)
...
class PriceLSTMRegressor(nn.Module):
    def __init__(self, input_dim: int, hidden_size: int, num_layers: int):
        ...
        self.out_activation = nn.Tanh()  # map to [-1, 1]
...
def load_csv(cfg: Config) -> pd.DataFrame:
    ...
    # Sort time for chronological dataset
...
def main():
    ...
    df = load_csv(cfg)
    dataset = PriceNewsDataset(df, cfg)
    ...
    torch.save(model.state_dict(), "price_lstm_with_news.pt")
    print("Saved model to price_lstm_with_news.pt")
