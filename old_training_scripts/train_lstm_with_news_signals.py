...
def main():
    ...
    best_epoch = 0
    best_val_sig_acc = -1.0
    best_val_sig_ret = 0.0
    best_thr = 0.0
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        if len(val_preds) == 0:
            ...
            for thr, sig_acc, sig_cov, sig_ret in thr_results:
                ...
                if sig_cov < cfg.min_sig_cov:
                    continue
                if sig_acc > best_sig_acc_epoch + 1e-6:
                    best_sig_acc_epoch = sig_acc
                    best_sig_cov_epoch = sig_cov
                    best_sig_ret_epoch = sig_ret
                    best_thr_epoch = thr
                elif abs(sig_acc - best_sig_acc_epoch) <= 1e-6 and sig_ret > best_sig_ret_epoch:
                    best_sig_acc_epoch = sig_acc
                    best_sig_cov_epoch = sig_cov
                    best_sig_ret_epoch = sig_ret
                    best_thr_epoch = thr
        ...
        improved = False
        if val_sig_acc > best_val_sig_acc + cfg.early_stopping_min_delta:
            improved = True
        elif abs(val_sig_acc - best_val_sig_acc) < 1e-6 and val_sig_ret > best_val_sig_ret:
            improved = True

        if improved:
            best_val_sig_acc = val_sig_acc
            best_val_sig_ret = val_sig_ret
            best_epoch = epoch
        ...
        if epochs_without_improvement >= cfg.early_stopping_patience:
            print(
                ...
                f"Best epoch was {best_epoch} with val_sig_acc={best_val_sig_acc:.3f}."
            )
            break
    ...
    if best_state_dict is not None:
        ...
        print(
            ...
            f"(val_sig_acc={best_val_sig_acc:.3f}, val_sig_ret={best_val_sig_ret:.4f})."
        )
    else:
